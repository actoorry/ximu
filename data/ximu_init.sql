-- ============================================================
-- ximu 完整建库脚本（初始化一个全新库）
-- 内容 = 合并 Flyway V1~V5 迁移后的最终 schema（含 request_id 幂等键、
--        material 五维唯一键、库存数量非负 CHECK、safe_stock 表、字段加长）+ 全部种子数据。
-- 用法：mysql -uroot -p < ximu_init.sql
-- 幂等：CREATE TABLE IF NOT EXISTS + INSERT IGNORE INTO，可重复执行。
-- 说明：本脚本为「全新库一次性初始化」用；存量库升级请走 Flyway 迁移（db/migration/V2~V5），勿用本脚本。
-- ⚠️ 警告：种子数据段含 DELETE 明细语句（按种子单号删除后重插），
--   若在已有真实数据的 ximu 库执行，会删除与种子单号相同的真实明细！切勿对生产/开发库直接执行。
-- ============================================================

CREATE DATABASE IF NOT EXISTS ximu DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE ximu;
...
--       (org_id, product_name, material, spec, grade) 五维建唯一索引，INSERT IGNORE 重复执行幂等。
--       作为全新库初始化示例数据。
-- ============================================================
