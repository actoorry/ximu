-- ============================================================
-- ⚠️ 已弃用（P2-15 起，R2-P2-31 更新）：本文件不再是有效初始化路径，仅作历史参考保留。
--    唯一 DDL 来源 = Flyway 迁移链（db/migration/V1~V10）；
--    本文件内容已滞后于迁移链（缺 V6~V10 的清理/索引/约束/审计列/删列变更），不要据此建库，
--    后续如需整库参考 DDL 应由迁移链在库上导出生成，不再手工维护本文件。
-- 注意：spring.sql.init.mode 恒为 never（开发也一律走 Flyway），本文件不会自动执行；
--       生产环境 DDL 由 Flyway（inventory-service 迁移权）或 DBA 流程管理。
-- ============================================================
